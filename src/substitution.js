...
const substitutionModule = (function () {
  // this function will both encode and decode using the substitution method
const letters = [
  "a",
  "b",
  "c",
  ...
  function substitution(input, alphabet, encode = true) {
    ...
    const repeatCheck = cipherAlphabet.reduce(
      (acc, character, index, collection) => {
        if (character in acc) {
          acc[character]++;
        } else {
          acc[character] = 1;
    ...
    if (!encode) {
      ...
      for (let index = 0; index < input.length; index++) {
        ...
        if (currentCharacterIndex === -1) {
          ...
          //console.log(currentCharacter, currentCharacterIndex)
